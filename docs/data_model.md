...
- `description` is the normalized human-readable description.
- `original_description` preserves the source text.
...
- Personal account CSV for January 2026.
...
- `source_file_hash` is required for file imports to support duplicate import detection.
...
- `raw_payload_json` should store source columns and values exactly as imported where practical.
