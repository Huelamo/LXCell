...
- Normal category editing should preserve the existing `canonical_key` when the
  visible name changes; technical key editing belongs in an advanced view.
...
- Manual transactions entered by the user are stored with `review_status = user_confirmed`.
- Manual transactions can still have `category_id = null` when the user has not classified them yet.
- Manual transaction workflows always create manual-entry audit records through `ImportBatch` and `ImportedTransactionSource`.
- Categorized manual transactions create accepted `manual_user` classification decisions.
- Uncategorized manual transactions do not create a classification decision until the user assigns a category.
...
- A user-entered transaction has already been intentionally reviewed enough to become part of the ledger.
- Missing category information should remain visible as uncategorized data, not as an automatic pending-review state.
...
- Add a minimal command line interface in `src/lxcell/cli.py`.
- Support `python -m lxcell.cli` rather than adding a package entry point in this block.
- Use `data/lxcell.db` as the default local SQLite path.
- Keep `data/` ignored by git.
- Support initializing the database schema.
- Support creating user profiles, accounts, and categories.
- Support recording manual transactions through `AccountingService`.
- Support listing transactions for a user profile.
- Parse user-entered decimal amounts into integer minor units without using floats.
- Keep CLI examples generic and privacy-safe.

Deferred:

- Streamlit or other graphical UI.
- CSV/bank/Excel importers.
- Interactive prompts.
- Editing existing records through the CLI.
- Packaging-level console script entry point.

Rationale:

- Manual entry unlocks practical local use before importers and UI are ready.
- Routing transaction creation through `AccountingService` preserves the accepted manual-entry audit trail.

## Block 14 - Local Streamlit UI

Accepted:

- Add a minimal Streamlit UI in `src/lxcell/ui/streamlit_app.py`.
- Use Spanish user-facing labels.
- Use `data/lxcell.db` as the default local SQLite path.
- Support creating user profiles, accounts, and categories.
- Support recording manual transactions through `AccountingService`.
- Support viewing transactions for a selected user profile.
- Support editing transactions directly in the transactions table, saving
  changes through `AccountingService`.
- Support soft deleting transactions through `AccountingService`.
- Support viewing and editing existing categories directly in a categories table,
  saving changes through `AccountingService`.
- Support removing categories from active use by setting `is_active = false`,
  not by hard deleting category rows.
- Hide inactive categories from the normal UI; show them only in the advanced
  category view.
- In the advanced category view, expose category lifecycle as read-only `estado`
  and editable `acción`, avoiding separate user-editable `activa` and `eliminar`
  fields.
- Keep financial logic out of the UI layer.
- Show a visible confirmation after successful writes.
- Ask for explicit confirmation before registering a manual transaction that
  exactly matches an existing transaction by date, account, category,
  description, amount, currency, direction, transaction type, and payment method.
- Do not hard delete transactions from the UI.
- Classification-changing edits supersede previous classification decisions and
  create a new accepted manual decision when a category is assigned.

Deferred:

- CSV/bank/Excel importers.
- Rich review queues.
- Budget and reporting dashboards.
- Final UI product design.

Rationale:

- Manual transaction entry is more useful through a form-based UI than a CLI.
- Reusing the service layer keeps manual-entry auditability consistent across UI and CLI.
